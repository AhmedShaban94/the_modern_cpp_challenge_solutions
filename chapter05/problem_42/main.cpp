...
unsigned int daysBetweenTwoDates(const date_t& date_1, const date_t& date_2) {
    if (date_1.ok() and date_2.ok())
        return std::abs(
            (date::sys_days{date_1} - date::sys_days{date_2}).count());
...
auto weekNumberOfTheYear(const date_t& date) {
    if (date.ok())
        return static_cast<unsigned int>(
            iso_week::year_weeknum_weekday{date}.weeknum());
    throw std::logic_error{"Date is invalid.\n"};
}

TEST_CASE("Given a date, get day number of the year",
          "[day_number_of_the_year]") {
    using namespace date::literals;
...
TEST_CASE("Given a date, get week number of the year",
