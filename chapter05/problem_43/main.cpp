...
void printMeetingsTimes(const date::zoned_time<Duration, TimeZonePtr>& time,
...
int main() {
    std::vector<user> users{user{"Budapest", "Europe/Budapest"},
    ...
    auto localtime = date::zoned_time<std::chrono::minutes>(
        date::current_zone(), static_cast<date::local_days>(today) +
                                  std::chrono::hours{h} +
