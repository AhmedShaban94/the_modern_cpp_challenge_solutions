...
void selectMovies(const std::vector<std::reference_wrapper<const query>>& queries, const std::string& filePath) {
  ...
  if (result) {
    for (const auto& query : queries) {
      try {
        const auto results = doc.select_nodes(query.get().searchString.c_str());
        for (const auto& res : results) {
          const auto str = res.node().attribute(query.get().attribute.c_str()).as_string();
    ...
    std::cout << "Error offset: " << result.offset << " (error at [..." << (filePath.c_str() + result.offset)
              << "]\n\n";
  }
}
...
int main() {
  ...
  for (const auto& title : movieTitles.queryResults) std::cout << '\t' << title << '\n';

  std::cout << movieRoles.attribute << ": \n";
  for (const auto& role : movieRoles.queryResults) std::cout << '\t' << role << '\n';
