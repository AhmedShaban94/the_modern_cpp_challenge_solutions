...
std::ostream& operator<<(std::ostream& os, const Movie& m) {
  ...
  os << "writers: " << '\n';
  for (const auto& writer : m.writers_) os << writer << '\n';

  os << "directors: \n";
  for (const auto& director : m.directors_) std::cout << director << '\n';

  os << "Cast: \n";
  for (const auto& role : m.cast_) os << "star: " << role.actorName_ << '\t' << "name: " << role.characterName_ << '\n';

  return os;
}

void serializeMovies(const std::vector<Movie>& movies, const std::string& xmlFileName) {
  ...
  for (const auto& movie : movies) {
    ...
    for (const auto& role : movie.cast_) {
      ...
      role_node.append_attribute("star").set_value(role.actorName_.c_str());
      role_node.append_attribute("name").set_value(role.characterName_.c_str());
    }
...
std::optional<std::vector<Movie>> deserializeMovies(std::string sourceFile) {
  ...
  if (result) {
    ...
    for (const auto& movieNode : root) {
      ...
      for (const auto& director : directorsNode) directors.push_back(director.attribute("name").as_string());
      m.directors_ = directors;

      const auto writersNode = movieNode.child("writers");
      std::vector<std::string> writers{};
      for (const auto& writer : writersNode) writers.push_back(writer.attribute("name").as_string());
      m.writers_ = writers;
    ...
    std::cout << "XML [" << sourceFile << "] parsed with errors, attr value: ["
              << doc.child("node").attribute("attr").value() << "]\n";
    std::cout << "Error description: " << result.description() << "\n";
    std::cout << "Error offset: " << result.offset << " (error at [..." << (sourceFile.c_str() + result.offset)
              << "]\n\n";
    return std::nullopt;
  }
}
...
int main() {
  ...
  for (const auto& movie : movies.value()) std::cout << movie << '\n';
  return EXIT_SUCCESS;
}