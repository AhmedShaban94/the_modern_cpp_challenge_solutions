...
enum class TEMP_CONVERT_UNITS
{
    TO_KELVIN,
    TO_FAHRENHEIT
};

long double operator""_deg(long double temp) { return temp; }
long double operator""_k(long double temp) { return temp - 273.15; }
long double operator""_f(long double temp) { return (temp - 32) * 0.5556; }

long double convert_temp(const long double &degree, const TEMP_CONVERT_UNITS &unit)
{
    if (unit == TEMP_CONVERT_UNITS::TO_KELVIN)
        return degree + 273.15;
    else if (unit == TEMP_CONVERT_UNITS::TO_FAHRENHEIT)
        return (degree / 0.5556) + 32;
    else
        return degree;
}

TEST_CASE("temperature conversion and user-defined literals", "[temp_conversion]")
{
