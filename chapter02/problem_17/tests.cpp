...
TEST_CASE("testing Array2D class", "[Array2D]") {
    ...
    SECTION("Testing constructors") {
        ...
        REQUIRE_NOTHROW(
            (Array2D<std::size_t, 3, 3>{{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}}));
    ...
    SECTION("Testing Array2D") {
        ...
        SECTION("Testing fill()") {
            ...
            std::for_each(arr.begin(), arr.end(),
                          [VALUE](const auto& ele) { REQUIRE(ele == VALUE); });
        }

        SECTION("Testing swap()") {
            Array2D<std::size_t, 3, 3> other{
                {11, 12, 13, 14, 15, 16, 17, 18, 19}};
            arr.swap(other);

            std::size_t value{11};
            std::for_each(arr.begin(), arr.end(), [&value](const auto& ele) {
                REQUIRE(ele == value++);
            });
        }

        SECTION("Testing dimension mismatch") {
            REQUIRE_THROWS_AS((Array2D<std::size_t, 3, 3>{{1, 2, 3}}),
                              std::runtime_error);
            REQUIRE_THROWS_AS(
                (Array2D<std::size_t, 1, 1>{{1, 2, 3, 4, 5, 6, 7, 8}}),
                std::runtime_error);
            REQUIRE_THROWS_AS(
                (Array2D<std::size_t, 1, 1>{{{1, 2, 3}, {4, 5, 6}, {7, 8}}}),
                std::runtime_error);
