   ...
   public:
    ...
    Array2D(const std::vector<std::vector<T>>& vec2d) {
        if (not(vec2d.size() == ROWS and vec2d.at(0).size() == COLS))
            throw std::runtime_error{"dimension mismatch"};
        std::for_each(vec2d.begin(), vec2d.end(), [this](const auto& row) {
            this->vec.insert(vec.end(), row.begin(), row.end());
        });
    }
    ...
    constexpr const T& operator()(const size_t& r,
    ...
    void print() const {
        for (size_t r = 0; r < ROWS; r++) {
            const std::vector<size_t> row(begin() + r * COLS,
                                               begin() + r * COLS + COLS);
            std::cout << "[ ";
