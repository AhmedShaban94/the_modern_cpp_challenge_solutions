...
class Array2D {
 ...
 public:
  ...
  Array2D(std::vector<T> vec)
      : vec{ROWS * COLS == vec.size() ? std::move(vec) : throw std::runtime_error{"dimension mismatch"}} {}

  Array2D(const std::initializer_list<T>& il)
      : vec{ROWS * COLS == il.size() ? il : throw std::runtime_error{"dimension mismatch"}} {}

  Array2D(const std::vector<std::vector<T>>& vec2d) {
    if (not(vec2d.size() == ROWS and vec2d.at(0).size() == COLS)) throw std::runtime_error{"dimension mismatch"};
    std::for_each(vec2d.begin(), vec2d.end(),
                  [this](const auto& row) { this->vec.insert(vec.end(), row.begin(), row.end()); });
  }
  ...
  [[nodiscard]] constexpr std::pair<size_t, size_t> size() const {
  ...
  void swap(Array2D& other) {
    std::swap(other.vec, this->vec);
  }

  void print() const {
    for (size_t r = 0; r < ROWS; r++) {
      const std::vector<size_t> row(begin() + r * COLS, begin() + r * COLS + COLS);
      std::cout << "[ ";
