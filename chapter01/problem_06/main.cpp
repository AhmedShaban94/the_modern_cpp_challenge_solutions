...
PairsVector abundantNumbers(const size_t& limit) {
  ...
  for (size_t i = 1; i < limit; ++i) {
    ...
    auto divisors_sum = std::accumulate(divisors.begin(), divisors.end(), (size_t)0);
    if (divisors_sum > i) nums.emplace_back(i, divisors_sum - i);
    divisors.clear();
  }
  return nums;
}
...
TEST_CASE("Get abundant numbers up to specific limit", "[abundant_numbers]") {
  PairsVector expected{{12, 4}, {18, 3}, {20, 2}, {24, 12}, {30, 12}, {36, 19}, {40, 10}, {42, 12}, {48, 28}, {54, 12}};
