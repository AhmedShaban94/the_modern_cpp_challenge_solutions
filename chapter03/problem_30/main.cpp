...
std::ostream& operator<<(std::ostream& os, const urlParts& parts) {
  ...
  if (parts.port.has_value()) os << "Port: " << parts.port.value() << '\n';
  if (parts.path.has_value()) os << "Path: " << parts.path.value() << '\n';
  if (parts.query.has_value()) os << "Query: " << parts.query.value() << '\n';
  if (parts.fragment.has_value()) os << "Fragment: " << parts.fragment.value() << '\n';
...
std::optional<urlParts> parse_url(const std::string& url) {
  ...
  if (found) {
    if (matches[1].matched and matches[2].matched) {
      ...
      if (matches[4].matched) parts.port.emplace(std::stoull(matches[4]));
      if (matches[5].matched) parts.path.emplace(matches[5].str());
      if (matches[7].matched) parts.query.emplace(matches[7].str());
      if (matches[9].matched) parts.fragment.emplace(matches[9].str());
